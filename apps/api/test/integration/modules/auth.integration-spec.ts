...
import { RegisterRequestDto } from 'src/modules/auth/schemas/dto/request/register.request.dto';
...
import { LoginRequestDto } from 'src/modules/auth/schemas/dto/request/login.request.dto';
import {
...
describe('Auth module integration', () => {
  ...
  describe('register', () => {
    ...
    it('should successfully orchestrate the user creation and its associated session in the database', async () => {
      const result = await authService.register(registerDto, metadata);
      ...
      expect(sessionInDb?.ipAddress).toBe(metadata.ipAddress);
      expect(sessionInDb?.userAgent).toBe(metadata.userAgent);

      // Verify the returned AuthEntity
      expect(result).toBeInstanceOf(AuthEntity);
      expect(result.user.email).toBe(userInDb?.email);
      expect(result.accessToken).toMatch(JWT_REGEX);
      expect(result.rawRefreshToken).toMatch(UUID_V4_REGEX);
  ...
  describe('refresh', () => {
    ...
    it('should refresh a session doing rotation : delete old, create new', async () => {
      const { rawRefreshToken: oldRawToken, user } =
        await authService.register(registerDto);
      ...
      expect(result.rawRefreshToken).not.toBe(oldRawToken); // rotation
  ...
  describe('logout', () => {
    it('should successfully remove the session from the database', async () => {
      ...
      const { rawRefreshToken, user } = await authService.register(registerDto);
