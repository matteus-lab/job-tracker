...
import { ISESSION_REPOSITORY_TOKEN } from 'src/modules/session/session.repository.interface';
import { CreateSessionPersistence } from 'src/modules/session/schemas/persistence/createSession.persistence';
import { CreateUserPersistence } from 'src/modules/user/schemas/persistence/createUser.persistence';
import { CreateSessionInput } from 'src/modules/session/schemas/inputs/createSession.input';
  ...
  describe('SessionService', () => {
    describe('create', () => {
      it('should create a real session in DB with fingerprint and correct expiration', async () => {
        ...
        expect(rawRefreshToken).toMatch(UUID_V4_REGEX);
    ...
    describe('validateSession', () => {
      it('should return a SessionEntity if the refresh token exists and is not expired', async () => {
        const { rawRefreshToken, sessionEntity } = await sessionService.create({
          userId: alreadyInsertedUserId,
          ipAddress: '127.0.0.1',
          userAgent: 'Test',
        });
    ...
    describe('deleteByRefreshToken', () => {
      it('should delete the correct session using the raw token', async () => {
        ...
        await sessionService.deleteByRefreshToken(rawRefreshToken);
